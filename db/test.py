...
data = '{"login": "143com", "password": "kasdjhvsauhewiviweb213"}'
resp = requests.post("https://dc76-62-183-34-186.ngrok-free.app/api/auth/signup", data=data, headers=headers)
print(resp.status_code, json.loads(resp.content))




data = '{"login": "a1321233@gmail.com", "password": "kasdjhvsauhewiviweb213"}'
resp = requests.post("https://dc76-62-183-34-186.ngrok-free.app/api/auth/login", data=data, headers=headers)
a = json.loads(resp.content)
headers["Authorization"] = f'Bearer {a["access"]}'
print(a["access"])


resp = requests.get("https://dc76-62-183-34-186.ngrok-free.app/api/auth", data=data, headers=headers)
a = json.loads(resp.content)
print(resp.status_code, resp.content)
